...
namespace TheCompleteProject.Api.FileUploadController
{
    public class FileUploadComplianceWithCustomResponse : BaseController
    {
        ...
        /// <summary>
        /// This Api Is Supporting Formulas Since We Were Getting Problems When A File Was Uploaded With Formulas So 
        /// With The Help Of FFC Refrence We Did This
        /// </summary>
        /// <param name="uploadUser"></param>
        /// <returns></returns>
        [HttpPost]
        [Route(("ExcelBulkImportWithCustomReturnTypeTwo"))]
        public async Task<Dictionary<string, object>> ExcelBulkImportWithCustomReturnTypeTwo([FromQuery] UploadUser uploadUser)
        {
            var strFileNameWithPath = uploadUser.file.FileName;
            var strFileType = Path.GetExtension(strFileNameWithPath);

            var path = FolderLocation.UPLOAD;
            var filePath = Path.Combine(path, uploadUser.file.FileName);
            filePath = filePath.Replace("\\", @"/");
            ...
            if (strFileType == ".xls" || strFileType == ".xlsx" || strFileType == ".XLS")
            {
                ...
                //var data = ExportImportHelper.GetDataTable(uploadUser.file);
                var data = await ExcelHelper.GetExcelData(uploadUser.file, filePath);
                if (data.Rows.Count < 0)
                    return FailureResponse("EMP113", null);
